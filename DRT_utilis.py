...
"""
This file is taken from the pyDRTTools project, https://github.com/ciuccislab/pyDRTtools
"""
def A_re(freq, tau):
...
def Simple_run(EIS_data, freq_ref, rbf_type = 'Gaussian', data_used = 'Combined Re-Im Data', induct_used = 1,
    ...
    # compute A matrix
    A_re_temp = assemble_A_re(freq, tau, epsilon, rbf_type)
    A_im_temp = assemble_A_im(freq, tau, epsilon, rbf_type)
    
    # compute M matrices
    if der_used == '1st order':
        M_temp = assemble_M_1(tau, epsilon, rbf_type)
    elif der_used == '2nd order':
        M_temp = assemble_M_2(tau, epsilon, rbf_type)
    ...
    if induct_used == 1: #considerating the inductance
        N_RL = 2
        A_re = np.zeros((N_freqs, N_taus+N_RL))
        A_re[:, N_RL:] = A_re_temp
        A_re[:,1] = 1
        
        A_im = np.zeros((N_freqs, N_taus+N_RL))
        A_im[:, N_RL:] = A_im_temp
        A_im[:,0] = 2*pi*freq

        M = np.zeros((N_taus+N_RL, N_taus+N_RL))
        M[N_RL:,N_RL:] = M_temp
    # add the code that link with the cross validation method
    
    # Step 2.2: Carry out ridge regression
    # these matrices are used for the constraint of the qp solver
    N_RL = 2
    lb = np.zeros([N_taus+N_RL,1])
    bound_mat = -np.eye(N_taus+N_RL)
    
    H_combined, c_combined = quad_format_combined(A_re, A_im, b_re, b_im, M, lambda_value)
    x = cvxopt_solve_qp(H_combined, c_combined, bound_mat, lb)
    # mu_Z_re = A_re@x
    # mu_Z_im = A_im@x
    ...
    A_re_ref = assemble_A_re(freq_ref, tau, epsilon, rbf_type)
    A_im_ref = assemble_A_im(freq_ref, tau, epsilon, rbf_type)
    Z_re_model = R_inf + np.matmul(A_re_ref, gamma_RR)
    Z_im_model = 2.*pi*freq_ref*L_0 + np.matmul(A_im_ref, gamma_RR)
    return Z_re_model+1j*Z_im_model